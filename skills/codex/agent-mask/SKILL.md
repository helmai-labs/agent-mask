---
name: agent-mask
description: Use when you need Codex to assume the role of a named KeelOS/OpenClaw agent from its live workspace and persist memory the same way the runtime does. This skill hydrates the agent from the runtime workspace, loads durable and recent memory, appends richer daily memory, records full-thread session transcripts, and only promotes durable facts into MEMORY.md.
---

# Agent Mask

## Purpose
Use this skill to wake a Codex thread up as a named KeelOS agent.

This is the closest Codex equivalent of starting a live OpenClaw agent:
...
- OpenClaw/Paperclip has true runtime session persistence
- this skill mirrors the workspace-file memory layer, not the adapter's hidden session state

## Memory Model
Mirror the OpenClaw split as closely as practical:
...
- `memory/YYYY-MM-DD.md` is the richer daily carry-forward surface. It should be more detailed than a terse note and can blend the old daily-memory summary style with the useful context that previously lived in session summaries.
...
## Use This Skill When
Use this skill when:
- the user says to assume the role of a named agent such as `david-a` or `jensen`
- a new thread should behave like an existing KeelOS/OpenClaw agent
- the user wants the thread to persist memory into the agent workspace
- the user wants one agent persona to be reusable across Codex and OpenClaw-style workflows

Do not use this skill when:
- the request is ordinary coding work with no agent-role hydration
- the user only wants a one-off style change with no workspace or memory persistence
- the named agent has no live runtime workspace and a plain fallback answer is better

## Workspace Resolution
Resolve the live runtime workspace in this order:

1. `/Users/cowboy/Projects/KeelOS/.keelos/runtime/agents/<agent>/workspace`
2. `.keelos/runtime/agents/<agent>/workspace`

On this machine, the shared `KeelOS` runtime path is the canonical file-backed source of truth when it exists.

Do not use `agents/<agent>/workspace` as a persistence fallback. Those repo-local copies are bootstrap or reference material and can drift from the live runtime memory.

If no live runtime workspace exists:
- say so briefly
- do not pretend the agent was loaded
- do not create or update `memory/` or `sessions/` inside `agents/<agent>/workspace`
...
All daily-memory and session writes must go to the resolved live runtime workspace only.
...
Use this for:
- the full thread transcript for each Codex or OpenClaw session that day
...
Examples:
- `Source: Codex Local Skill`
- `Source: Claude Local Skill`
...
## Codex Local Skill Session - YYYY-MM-DD
...
For `sessions/YYYY-MM-DD-HHMMSS-codex-local-skill-agent-mask.md`:

```md
# Session - YYYY-MM-DD - Codex Local Skill - agent-mask - <agent>
...
### 1. Hydrate
Resolve the live runtime workspace and read the startup files in order.
...
- uses the canonical live runtime workspace rather than a repo-local fallback copy
...
- writes memory into `agents/<agent>/workspace` while a live runtime workspace exists elsewhere
...
- `$linear-task-writing` when the hydrated agent is doing backlog work
- `$keelos-execution-briefs` or `$keelos-technical-validation` when the hydrated agent is David or Jensen working tickets
